...
namespace Retinues.Features.Staging
{
    ...
    /// </summary>
    [SafeClass]
    public abstract class BaseStagingBehavior<T> : CampaignBehaviorBase
        where T : IPendingData
    {
            ...
            foreach (var kvp in _pending)
            {
                ...
                foreach (var kvp2 in dict)
                {
                    ...
                    string disabledReason = eligible
                        ? null
                        : ContextManager.GetContextReason(troop, Instance.ActionString)?.ToString();
        ...
        protected static bool CanEdit(WCharacter troop)
        {
            return Config.RestrictEditingToFiefs == false
                || ContextManager.IsAllowedInContext(troop, Instance.ActionString);
        }
    }
}
