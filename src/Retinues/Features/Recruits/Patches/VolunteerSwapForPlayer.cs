...
namespace Retinues.Features.Recruits.Patches
{
    ...
    internal static class VolunteerSwapForPlayer
    {
        ...
        private static string _snapshotSettlementId;
        ...
        private static void SnapshotVolunteers(Settlement settlement)
        {
            ...
            if (string.IsNullOrEmpty(settlementId) || notables == null || notables.Count == 0)
                return;
            ...
            _snapshotSettlementId = settlementId;
            _snapshot = snapshot;
        }
        ...
        /// </summary>
        private static void RestoreSnapshot(string settlementId)
        {
            ...
            if (
                _snapshot == null
                || !string.Equals(_snapshotSettlementId, settlementId, StringComparison.Ordinal)
            )
                return;
            ...
            foreach (var notable in notables)
            {
                ...
                if (!_snapshot.TryGetValue(notable.StringId, out var snapshotVolunteers))
                    continue;
                ...
                var count = Math.Min(volunteers.Length, snapshotVolunteers.Length);
                for (var i = 0; i < count; i++)
                {
                    if (volunteers[i] == null)
                        continue;

                    volunteers[i] = snapshotVolunteers[i];
                }
            }

            ClearSnapshot();
            Log.Debug(
                "[VolunteerSwapForPlayer] Restored volunteers for settlement " + settlementId
            );
        }
        ...
        private static class VolunteerSwapForPlayer_WaitStart
        {
            ...
            {
                RestoreSnapshot(_snapshotSettlementId);
            }
        }
