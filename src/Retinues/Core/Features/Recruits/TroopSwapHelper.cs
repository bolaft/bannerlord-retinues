...
namespace Retinues.Core.Features.Recruits
{
    ...
    public static class TroopSwapHelper
    {
        ...
        {
            if (!IsValid(troop) || troop.Culture?.Base?.EliteBasicTroop == null)
                return false;
        ...
        {
            if (!IsValid(vanilla) || faction == null)
                return null;
        ...
        {
            if (faction == null || !IsValid(troop))
                return false;

            bool idMatch(string id) => id == troop.StringId;

            if (IsValid(faction.RetinueElite) && idMatch(faction.RetinueElite.StringId))
                return true;
            if (IsValid(faction.RetinueBasic) && idMatch(faction.RetinueBasic.StringId))
                return true;
            if (IsValid(faction.RootElite) && idMatch(faction.RootElite.StringId))
                return true;
            if (IsValid(faction.RootBasic) && idMatch(faction.RootBasic.StringId))
                return true;
        ...
        public static WCharacter FindReplacement(
            WCharacter vanilla,
            WFaction faction,
            bool militia = false
        )
        {
            ...
            if (militia)
            {
                ...
                foreach (var c in candidates)
                {
                    ...
                    if (c.Tier != vanilla.Tier)
                        continue;
                    if (c.IsRanged != vanilla.IsRanged)
                        continue;
                    if (c.IsElite != vanilla.IsElite)
                        continue;
        ...
        public static void SwapParty(MobileParty p)
        {
            ...
            catch (Exception ex)
            {
                Log.Exception(
                    ex,
                    $"TroopSwapHelper: Failed swapping party '{(p?.Name != null ? p.Name.ToString() : "NULL")}'."
                );
                Log.Error(
                    $"Clan: {p.ActualClan?.Name} ({p.ActualClan?.StringId}) Kingdom: {p.ActualClan?.Kingdom?.Name} ({p.ActualClan?.Kingdom?.StringId})"
                );
            }
        }

        public static void SwapRoster(WParty party, WFaction faction)
        {
            int replaced = 0;

            if (party.Base == null || party.MemberRoster == null || faction == null)
                return;
            ...
            foreach (var e in party.MemberRoster.Base.GetTroopRoster())
            {
                ...
                if (
                    ...
                    || replacement.StringId == vanilla.StringId
                )
                {
                    dst.AddToCounts(
                        ch,
            ...
            Log.Debug(
                $"TroopSwapHelper.SwapRoster: Members swapped for {party?.Name}; stacks replaced={replaced}."
            );
        }
    }
}
