...
namespace Retinues.Core.Features.Xp.Patches
{
    ...
    {
        private static readonly FieldInfo _fiRoster =
            AccessTools.Field(typeof(MapEventParty), "_roster");

        private static readonly Type _fteType =
            AccessTools.TypeByName("TaleWorlds.CampaignSystem.Roster.FlattenedTroopRosterElement");

        private static readonly PropertyInfo _fteTroop   = _fteType != null ? AccessTools.Property(_fteType, "Troop")    : null;
        private static readonly PropertyInfo _fteKilled  = _fteType != null ? AccessTools.Property(_fteType, "IsKilled") : null;
        ...
        static void Postfix(MapEventParty __instance)
        {
            try
            {
                ...
                foreach (var row in (System.Collections.IEnumerable) flattened)
                {
                    ...
                    var explained = model.CalculateXpGainFromBattles((FlattenedTroopRosterElement)row, party);
