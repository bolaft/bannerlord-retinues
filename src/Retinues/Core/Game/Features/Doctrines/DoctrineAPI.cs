...
namespace Retinues.Core.Game.Features.Doctrines
{
    public static class DoctrineAPI
    {
        ...
        public static DoctrineDef GetDoctrine<TDoctrine>() where TDoctrine : Doctrine
            => GetDoctrine(typeof(TDoctrine));

        /// Get a doctrine definition by type.
        public static DoctrineDef GetDoctrine(Type doctrineType)
        {
            if (doctrineType == null) return null;
        ...
        {
            if (!EnsureSvc(out var svc) || string.IsNullOrEmpty(doctrineKey)) return null;
        ...
        public static bool TryAcquireDoctrine<TDoctrine>(out string reason) where TDoctrine : Doctrine
            => TryAcquireDoctrine(typeof(TDoctrine), out reason);
        ...
        {
            if (!EnsureSvc(out var svc) || string.IsNullOrEmpty(featKey)) return 0;
        ...
        {
            if (!EnsureSvc(out var svc) || string.IsNullOrEmpty(featKey)) return false;
            return svc.IsFeatComplete(featKey);
        }

        public static void SetFeatProgress(string featKey, int amount)
        {
            if (!EnsureSvc(out var svc) || string.IsNullOrEmpty(featKey)) return;
        ...
        {
            if (!EnsureSvc(out var svc) || listener == null) return;
