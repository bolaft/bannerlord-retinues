...
namespace Retinues.Core.Game.Wrappers.Cache
{
    public static class WCharacterIndex
    {
        ...
        public static void Rebuild()
        {
            ...
            if (Player.Kingdom != null) sources.Add(Player.Kingdom); // Player.Kingdom is cached wrapper too. :contentReference[oaicite:5]{index=5}
            ...
            foreach (var fac in sources)
            {
                ...
                if (fac.RetinueBasic != null) _factionByTroopId[fac.RetinueBasic.StringId] = fac;
                if (fac.RetinueElite != null) _factionByTroopId[fac.RetinueElite.StringId] = fac;
            }
        }

        public static bool TryGetFactionByTroopId(string id, out WFaction fac) => _factionByTroopId.TryGetValue(id, out fac);
        public static bool TryGetParentId(string id, out string parentId) => _parentIdByChildId.TryGetValue(id, out parentId);
