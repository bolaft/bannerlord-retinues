...
namespace Retinues.Game
{
    ...
    public sealed class BootstrapBehavior : BaseCampaignBehavior<BootstrapBehavior>
    {
        ...
        {
            Dictionary<int, int> highestValueByTier = [];
            Dictionary<int, float> highestWeightByTier = [];
            string[] validCultures =
            [
                "nord",
                "empire",
                "sturgia",
            ...
            {
                if (wc.StringId == null || !validCultures.Contains(wc.StringId))
                    continue;
                foreach (var troop in wc.RootElite.Tree)
                {
                    int tier = troop.Tier;
                    foreach (var eq in troop.Equipments)
                    {
                        float weight = eq.Weight;
                        int value = eq.Value;

                        if (
                            !highestWeightByTier.ContainsKey(tier)
                            || weight > highestWeightByTier[tier]
                        )
                            highestWeightByTier[tier] = weight;

                        if (
                            !highestValueByTier.ContainsKey(tier)
                            || value > highestValueByTier[tier]
                        )
                            highestValueByTier[tier] = value;
                    }
                }
                foreach (var troop in wc.RootBasic.Tree)
                {
                    int tier = troop.Tier;
                    foreach (var eq in troop.Equipments)
                    {
            ...
            Log.Info("Computed highest equipment limits by tier:");
            for (int tier = 0; tier <= 7; tier++)
            {
                float weight = highestWeightByTier.ContainsKey(tier)
                    ? highestWeightByTier[tier]
                    : 0f;
                int value = highestValueByTier.ContainsKey(tier) ? highestValueByTier[tier] : 0;

                Log.Info($" - Tier {tier}: Weight = {weight:0.0} kg, Value = {value} denars");
            }
