...
namespace Retinues.Compatibility.Interops.OldRealms
{
    ...
    /// troops use generated stub ids that TOR has no entry for, so they lose those attributes in
    /// battle.
    ///
    /// Each custom troop is cloned from a source troop (its <see cref="WCharacter.SourceStringId"/>),
    /// which — for a TOR-based troop — has an entry. We copy the source's entry onto the custom id
    /// at runtime: the automatic equivalent of the community's manual edit to
    /// tor_extendedunitproperties.xml. All reflection here is best-effort and no-ops when TOR is
    /// not installed.
    /// </summary>
    [SafeClass]
    internal static class OldRealmsExtendedInfo
    {
        ...
        /// <summary>
        /// Copies source TOR extended info onto every active custom troop id. Safe to call
        /// repeatedly (it overwrites, so re-based troops and TOR registry reloads stay in sync).
        /// </summary>
        public static void Sync()
        {
            if (!Mods.OldRealms.IsLoaded || !Resolve())
                return;
            ...
            foreach (var troop in WCharacter.All)
            {
                try
                {
                    ...
                    var templateId = troop.SourceStringId;
                    if (string.IsNullOrEmpty(templateId) || templateId == id)
                        continue;

                    // Only sync troops whose source actually has TOR extended info.
                ...
                {
                    Log.Exception(e, $"OldRealms: failed to sync extended info for '{troop?.StringId}'.");
                }
            }
        }

        /// <summary>
        /// Shallow-clones a CharacterExtendedInfo and stamps it with the custom troop id. The inner
        /// lists (abilities/attributes/resistances) are loaded-from-XML and read-only, so sharing
        /// their references is safe.
        /// </summary>
